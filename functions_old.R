...
#' @param col_means vector of expected values of the sample
#' @param critical Critical value for the F distribution
#' @param binorm_data sample data
#' @param n number of samples
#' @param p number of parameters we are estimating
#' 
T_hotelling <- function(mu, col_means, critical, binorm_data, n, p){
  n * (t(col_means - mu) %*% solve(cov(binorm_data)) %*% (col_means - mu)) * ((n - p)/(p * (n - 1))) - critical
}

#' Hotelling test statistic radial form
#' 
#' @param r distance
#' @param phi radial component
#' @param critical Critical value for the F distribution 
#' @param binorm_data sample data
#' @param n number of samples
#' @param p number of parameters we are estimating
#' 
T_hotelling_radial <- function(r, phi, critical, binorm_data, n, p){
  n * (t(r * c(cos(phi), sin(phi))) %*% solve(cov(binorm_data)) %*% (r * c(cos(phi), sin(phi)))) * ((n - p)/(p * (n - 1))) - critical
}
...
confRegionBoundary <- function(num_points, alpha_value, sigma_x = NULL, sigma_y = NULL, mu_x, mu_y, rho = NULL, n, label = NULL, Sigma = NULL){
  # extract sd and rho
  if(!is.null(Sigma)){
